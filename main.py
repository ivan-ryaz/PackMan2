...
class levelBase(object):
    ...
    def GhostsA(self, ghosts, nodes):
        ...
        for i in list(self.ghostNodeDeny.keys()):
            for j in self.ghostNodeDeny[i]:
                nodes.denyAccessList(*(j + (i, ghosts)))
...
class NodeG(object):
    def __init__(self, level):
        ...
        self.pathSymbols = ['.', '-', '|', 'p']
        data = self.readlvl(level)
        self.createNodeTable(data)
        self.connectHorizontally(data)
        self.connectVertically(data)
        self.homekey = None

    def readlvl(self, textfile):
...
class TextGroup(object):
    ...
    def setupText(self):
        ...
        with open('score/Hiscore.txt', 'r') as s:
            self.alltext[hiscr] = Text(s.read().zfill(8), (255, 215, 0), 11 * twidth, 20, size)
        self.addText('SCORE', (255, 255, 255), 0, 0, size)
        self.addText('LEVEL', (255, 255, 255), 23 * twidth, 0, size)
        self.addText('HISCORE', (255, 215, 0), 11 * twidth, 0, size)
...
class levelSprites(Spritesheet):
    def __init__(self, levelfile, rotfile):
        Spritesheet.__init__(self)
        self.data = self.readlvl(levelfile)
        self.rotdata = self.readlvl(rotfile)

    def getImage(self, x, y):
        return Spritesheet.getImage(self, x, y, twidth, theight)

    def readlvl(self, levelfile):
...
class GameOver(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load('sprites/GameOver.jpg')
        left, top = self.image.get_rect()[:2]
        self.top = top
        self.left = -screenSize[0]
        f = self.image.get_rect()
        f.right = 9
...
class PackMan(object):
    def __init__(self):
        ...
        fon = pygame.image.load("sprites/Заставка.jpg")
    ...
    def start(self):
        self.lvldata.loadlevel(self.level)
        self.levelsprites = levelSprites('levels/' + self.lvldata.obj.name + '.txt',
                                       'levels/' + self.lvldata.obj.name + '_rotation.txt')
        self.setBG()
        self.nodes = NodeG('levels/' + self.lvldata.obj.name + '.txt')
        self.lvldata.obj.setPortalPairs(self.nodes)
        self.lvldata.obj.conHomnod(self.nodes)
        self.pacman = Pacman(self.nodes.getNodeFromTiles(*self.lvldata.obj.pacmanStart))
        self.pellets = PelletGroup('levels/' + self.lvldata.obj.name + '.txt')
        self.ghosts = GhostGroup(self.nodes.getStartTempNode(), self.pacman)
        self.ghosts.pinky.setStartNode(self.nodes.getNodeFromTiles(*self.lvldata.obj.addOffset(2, 3)))
        self.ghosts.inky.setStartNode(self.nodes.getNodeFromTiles(*self.lvldata.obj.addOffset(0, 3)))
        self.ghosts.clyde.setStartNode(self.nodes.getNodeFromTiles(*self.lvldata.obj.addOffset(4, 3)))
        self.ghosts.setSpawnNode(self.nodes.getNodeFromTiles(*self.lvldata.obj.addOffset(2, 3)))
        self.ghosts.blinky.setStartNode(self.nodes.getNodeFromTiles(*self.lvldata.obj.addOffset(2, 0)))

        self.nodes.denyHomeAccess(self.pacman)
        self.nodes.denyHomeAccessList(self.ghosts)
        self.ghosts.inky.startNode.denyAccess(right, self.ghosts.inky)
        self.ghosts.clyde.startNode.denyAccess(left, self.ghosts.clyde)
        self.lvldata.obj.GhostsA(self.ghosts, self.nodes)
                        ...
                        if self.lives <= 0:
                            ...
                            while running:
                                ...
                                for event in pygame.event.get():
                                    if event.type == pygame.QUIT:
                                        running = False
                                game.screen.blit(hero.image, hero.move(a))
                                pygame.display.flip()
                                clock1.tick(fps)
                                b += 1
                            run = True
                            while run:
                                for event in pygame.event.get():
                                    if event.type == pygame.QUIT or event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN:
                                        run = False
                            self.pause.setPause(pauseTime=0, func=self.restart)
    ...
    def newScore(self, points):
        self.score += points
        with open('score/Hiscore.txt', 'r') as sc:
            if int(sc.read()) < self.score:
                with open('score/Hiscore.txt', 'w') as newsc:
                    newsc.write(str(self.score))
                    self.TG.alltext[hiscr] = Text(str(self.score).zfill(8), (255, 215, 0), 11 * twidth, 20, theight)
        self.TG.newScore(self.score)
...
if __name__ == '__main__':
    ...
    pygame.display.set_icon(pygame.image.load("sprites/pack.png"))
    ...
    btiheignt = 16
    dth = 5
    SoundDeath = pygame.mixer.Sound('sounds/death.mp3')
    SoundTheme = pygame.mixer.Sound('sounds/theme.mp3')
    SoundWaka = pygame.mixer.Sound('sounds/wakawaka.mp3').play(-1)
